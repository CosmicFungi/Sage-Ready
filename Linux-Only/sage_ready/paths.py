...
def validate_comfy_path_for_host(value: str) -> str:
    ...
    if looks_like_windows_path(cleaned) and not host.startswith("win"):
        raise ValueError(
            ...
            "Paste a local Linux path such as /home/you/ComfyUI, or run Sage Ready "
            "on the Windows PC that has ComfyUI "
            "(example portable: B:\\ComfyUI_windows_portable\\ComfyUI)."
        )
    return cleaned
