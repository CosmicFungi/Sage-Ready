...
def _candidate_pythons(comfy_root: Path, main_py: Path) -> list[tuple[str, Path, int]]:
    ...
    Priority bands:
      ...
      1 = .venv / venv inside / beside ComfyUI (not an unrelated parent home venv)
      2 = VIRTUAL_ENV / CONDA_PREFIX (active shells — risky for wrong-env)
      3 = system / current interpreter (last resort)
    """
    candidates: list[tuple[str, Path, int]] = []
    system = platform.system().lower()
    # Only search ComfyUI root and the folder that contains main.py — not an
    # arbitrary grandparent that might be $HOME with an unrelated .venv.
    roots: list[Path] = []
    for r in (comfy_root, main_py.parent):
        try:
            resolved = r.resolve()
        except OSError:
            continue
    ...
    # Portable layout: ComfyUI/ inside ComfyUI_windows_portable/ — allow parent
    # only when that parent looks like a ComfyUI install root (has python_embeded
    # or a nested ComfyUI/main.py already handled via roots).
    try:
        parent = comfy_root.resolve().parent
    except OSError:
        parent = None
    seen: set[Path] = set()
    ...
    def add_venv_bins(base: Path, priority: int) -> None:
        if system == "windows":
            add("venv", base / "Scripts" / "python.exe", priority)
        else:
            add("venv", base / "bin" / "python", priority)
            add("venv", base / "bin" / "python3", priority)
    ...
    for root in roots:
        if system == "windows":
            ...
            for name in (".venv", "venv"):
                add_venv_bins(root / name, 1)
                # Parent venv only when parent hosts portable python_embeded
                # (typical Windows portable layout), not a random home folder.
                if parent is not None and (
                    (parent / "python_embeded").is_dir()
                    or (parent / "python_embedded").is_dir()
                ):
                    add_venv_bins(parent / name, 1)
        else:
            for name in (".venv", "venv"):
                add_venv_bins(root / name, 1)
    ...
    for env_var, env_type in (("VIRTUAL_ENV", "venv"), ("CONDA_PREFIX", "conda")):
        ...
        if system == "windows":
            ...
            add(
                env_type if env_type != "venv" else "active_venv",
...
def resolve_python(comfy_root: Path, main_py: Path) -> tuple[str, Path, bool]:
    ...
    if not candidates:
        raise FileNotFoundError(
            ...
            "On Windows portable, expect python_embeded/python.exe or a .venv."
        )
