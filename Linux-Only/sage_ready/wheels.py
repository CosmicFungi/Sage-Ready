...
def _torch_mm_tuple(torch_version: str) -> Optional[tuple[int, int]]:
    _, major_minor, _ = parse_torch_version(torch_version)
    try:
        major, minor = [int(x) for x in major_minor.split(".")[:2]]
        return major, minor
...
def _newer_than_windows_matrix(torch_version: str) -> bool:
    """True when torch is newer than curated Windows wheel ceiling (2.13)."""
    matrix = load_matrix()
    ceiling = matrix.get("supported_torch_max_windows", "2.13")
    detected = _torch_mm_tuple(torch_version)
    try:
        c_major, c_minor = [int(x) for x in ceiling.split(".")[:2]]
    except ValueError:
        return False
    if detected is None:
        return False
    return detected > (c_major, c_minor)


def _cuda_alias_note(env: EnvSnapshot) -> str:
    cuda = cuda_code_from_env(env)
    if not cuda:
        return ""
    aliases = load_matrix().get("cuda_aliases", {})
    if cuda in aliases:
        return (
            f" Detected CUDA {cuda} maps to the {aliases[cuda]} compatibility line "
            f"(e.g. cu132 → cu130)."
        )
    return ""
...
def plan_install(env: EnvSnapshot, host_platform: Optional[str] = None) -> WheelPlan:
    ...
    newer_torch = _newer_than_windows_matrix(torch_ver)
    alias_note = _cuda_alias_note(env)
    ...
    if not is_windows:
        notes = (
            "Linux/macOS install plan: pip sageattention==2.2.0 "
            "(--no-build-isolation; may need a CUDA toolkit / nvcc), "
            "then fall back to sageattention==1.0.6 if the build fails. "
            "Curated Windows prebuilt wheels are not used. "
            "Community Hugging Face Linux wheels are unsupported by this app."
        )
        if newer_torch:
            notes += (
                f" PyTorch {torch_ver} is newer than the Windows wheel matrix "
                f"(max {matrix.get('supported_torch_max_windows', '2.13')}); "
                "this host still uses the pip/build path."
            )
        notes += alias_note
        notes += f" Triton constraint for this torch: {constraint}."
        return WheelPlan(
            strategy=matrix.get("linux_strategy", "pip_sa2_or_fallback"),
            sage_version="2.2.0",
    ...
    if newer_torch:
        notes += (
            f" PyTorch {torch_ver} is above the curated Windows ceiling "
            f"({matrix.get('supported_torch_max_windows', '2.13')})."
        )
    notes += alias_note
    return WheelPlan(
...
def needs_version_upgrade(env: EnvSnapshot, plan: WheelPlan) -> bool:
    """True when a better planned SA2 package exists than what is installed."""
    if is_known_bad_version(env.sageattention_version):
        return True
    if plan.strategy != "wheel":
        # Linux/macOS pip path: nudge Repair when only 1.x is installed but plan wants 2.x
        if plan.strategy == "pip_sa2_or_fallback" and env.sageattention_version:
            installed = parse_version_tuple(env.sageattention_version)
            wanted = parse_version_tuple(plan.sage_version)
