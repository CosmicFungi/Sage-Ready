(() => {
  ...
  const PATH_SCHEMA = "3"; // bump: clear Windows paths saved on Linux hosts
  ...
  function isExampleOrStalePath(path) {
    ...
    if (t.includes("/home/you/")) return true;
    // Windows drive / UNC paths are useless on Linux/macOS hosts
    if (/^[a-z]:[\\/]/.test(t) || t.startsWith("\\\\")) return true;
    return false;
  }
  ...
  async function performScan() {
    ...
    // Persist path only after a successful HTTP response (not Windows paths on Linux)
    if (!isExampleOrStalePath(comfy_path)) {
    ...
    try {
      ...
      if (data.cloud_blocked || data.status === "cloud_blocked") {
        ...
        warn.textContent =
          ...
          "\n\nDownload the app onto the PC where ComfyUI lives and run:  python app.py\n" +
      ...
      let osLabel = data.platform || "PC";
      if (data.is_windows) osLabel = "Windows";
      else if (plat.includes("linux")) osLabel = "Linux";
      else if (plat.includes("darwin") || plat.includes("mac")) osLabel = "macOS";

      badge.textContent = `Local ${osLabel} · v${data.version}`;

      // Platform-aware path placeholder / hint
      const hint = $("path-hint");
      if (data.is_windows) {
        pathInput.placeholder = "example: B:\\ComfyUI_windows_portable\\ComfyUI";
        if (hint) {
          hint.innerHTML =
            "Choose the folder that contains <code>main.py</code>. Example: " +
            "<code>B:\\ComfyUI_windows_portable\\ComfyUI</code> (portable root or the inner " +
            "ComfyUI folder both work). We look for <code>python_embeded</code>—ComfyUI’s spelling—" +
            "so packages go into the right Python.";
        }
      } else {
        pathInput.placeholder = "example: /home/you/ComfyUI";
        if (hint) {
          hint.innerHTML =
            "Choose the folder that contains <code>main.py</code>. Example: " +
            "<code>/home/you/ComfyUI</code>. Prefer a <code>.venv</code> or <code>venv</code> " +
            "beside ComfyUI so packages install into the same Python ComfyUI uses.";
        }
      }
      ...
      if (!localPage) {
        ...
        warn.textContent =
          "Wrong URL. Sage Ready is local-only — open http://127.0.0.1:8765 after running python app.py on your PC.";
        pathInput.disabled = true;
        btnScan.disabled = true;
      } else if (!data.is_windows) {
        // Linux / macOS is supported — informational tip, not a block
        badge.classList.remove("is-remote");
        warn.classList.remove("hidden");
        warn.classList.add("is-info");
        const modeLabel = osLabel === "macOS" ? "macOS" : "Linux";
        warn.textContent =
          `${modeLabel} mode: paste a local ComfyUI path (example: /home/you/ComfyUI). ` +
          "Prefer a .venv beside ComfyUI. Install uses pip SageAttention 2.2.0 " +
          "(may need a CUDA toolkit), then falls back to 1.0.6. " +
          "PyTorch 2.14 / CUDA 13.x are supported for detection; curated Windows wheels do not apply here.";
      } else {
        badge.classList.remove("is-remote");
      ...
      if (footer) {
        footer.innerHTML =
          `Sage Ready · ${osLabel} · by ` +
          `<a href="https://github.com/CosmicFungi" rel="noopener noreferrer">CosmicFungi</a>` +
          ` · MIT License · v${data.version}`;
      }
    } catch {
